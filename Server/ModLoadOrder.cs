...
public class ModLoadOrder
{
    ...
    {
        this.mods = mods;
        modsAvailable = JsonSerializer.Deserialize<Dictionary<string, PackageJsonData>>(JsonSerializer.Serialize(this.mods));
        loadOrder = [];
    ...
    protected void GetLoadOrderRecursive(string mod, HashSet<string> visited)
    {
        ...
        var dependencies = new HashSet<string>(config.Dependencies.Keys);
