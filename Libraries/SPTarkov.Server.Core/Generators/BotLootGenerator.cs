    ...
    )
    {
        ...
        if (
            (botInventory.Items ?? []).Any(item =>
                item.SlotId == EquipmentSlots.TacticalVest.ToString()
            )
        )
        {
        ...
        if (
            (botInventory.Items ?? []).Any(item =>
                item.SlotId == EquipmentSlots.Backpack.ToString()
            )
        )
        {
    ...
    )
    {
        ...
        for (var i = 0; i < randomisedWeaponCount; i++)
        {
            ...
            if (result != ItemAddedResult.SUCCESS)
            {
                ...
                {
                    _logger.Debug(
                        $"Failed to add additional weapon {generatedWeapon.Weapon[0].Id} to bot backpack, reason: {result.ToString()}"
                    );
                }
            }
        }
