...
)
{
    ...
    public bool HasItemWithTpl(List<Item> itemPool, string item, string slotId = null)
    {
        ...
        var filteredPool = slotId is not null
            ? itemPool.Where(item => item.SlotId?.StartsWith(slotId) ?? false)
            : itemPool;
    ...
    public Item GetItemFromPoolByTpl(List<Item> itemPool, string item, string slotId = null)
    {
        ...
        return filteredPool.FirstOrDefault(poolItem =>
            poolItem.Template.Equals(item, StringComparison.OrdinalIgnoreCase)
        );
    }
    ...
    public bool ArmorItemHasRemovablePlateSlots(string itemTpl)
    {
        ...
        return itemTemplate.Value.Properties.Slots.Any(slot =>
            plateSlotIds.Contains(slot.Name.ToLower())
        );
    }
    ...
    public int[][] GetBlankContainerMap(int containerY, int containerX)
    {
        ...
        return Enumerable.Range(0, containerY).Select(i => new int[containerX]).ToArray();
    }
}
