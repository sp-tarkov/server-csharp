...
)
{
    ...
    )
    {
        ...
        if (!directReward.Repeatable) FlagDirectRewardAsAcceptedInProfile(sessionId, directReward);
        ...
        CultistCircleSettings cultistCircleConfig)
    {
        ...
        if (cultistCircleConfig.AdditionalRewardItemPool.Count > 0)
            ...
            {
                if (itemRewardBlacklist.Contains(additionalReward)) continue;
        ...
        HashSet<string> rewardPool)
    {
        ...
        foreach (var task in activeTasks)
        {
            ...
            {
                if (itemRewardBlacklist.Contains(neededItem) || !_itemHelper.IsValidItem(neededItem)) continue;

                if (_logger.IsLogEnabled(LogLevel.Debug)) _logger.Debug($"Added Task Loot: {_itemHelper.GetItemName(neededItem)}");
        ...
        foreach (var profileArea in GetPlayerAccessibleHideoutAreas(pmcData.Hideout.Areas))
        {
            ...
            if (nextStageDbData is not null)
            {
                ...
                foreach (var rewardToAdd in itemRequirements)
                {
                    ...
                    if (_logger.IsLogEnabled(LogLevel.Debug)) _logger.Debug($"Added Hideout Loot: {_itemHelper.GetItemName(rewardToAdd.TemplateId)}");
        ...
        bool itemsShouldBeHighValue)
    {
        ...
        )
        {
            ...
            if (itemRewardBlacklist.Contains(randomItem.Id) || !_itemHelper.IsValidItem(randomItem.Id)) continue;

            // Valuable check
            if (itemsShouldBeHighValue)
            {
                var itemValue = _itemHelper.GetItemMaxPrice(randomItem.Id);
                if (itemValue < _hideoutConfig.CultistCircle.HighValueThresholdRub) continue;
            }

            if (_logger.IsLogEnabled(LogLevel.Debug)) _logger.Debug($"Added: {_itemHelper.GetItemName(randomItem.Id)}");
