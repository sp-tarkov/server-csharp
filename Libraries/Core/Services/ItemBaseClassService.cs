...
)
{
    ...
    public void HydrateItemBaseClassCache()
    {
        ...
        foreach (var item in filteredDbItems)
        {
            ...
            if (!_itemBaseClassesCache.ContainsKey(item.Value.Id)) _itemBaseClassesCache[item.Value.Id] = [];
    ...
    protected void AddBaseItems(string itemIdToUpdate, TemplateItem item)
    {
        ...
        if (!string.IsNullOrEmpty(parent.Parent)) AddBaseItems(itemIdToUpdate, parent);
    }
    ...
    {
        if (!_cacheGenerated) HydrateItemBaseClassCache();
        ...
        if (!CachedItemIsOfItemType(itemTpl)) return false;

        // No item in cache
        if (_itemBaseClassesCache.ContainsKey(itemTpl)) return _itemBaseClassesCache[itemTpl].Any(baseClasses.Contains);

        if (_logger.IsLogEnabled(LogLevel.Debug)) _logger.Debug(_localisationService.GetText("baseclass-item_not_found", itemTpl));
    ...
    public List<string> GetItemBaseClasses(string itemTpl)
    {
        ...
        if (!_itemBaseClassesCache.ContainsKey(itemTpl)) return [];
